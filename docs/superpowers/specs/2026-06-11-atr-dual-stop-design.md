...
- `RISK_PER_TRADE_USD`/`MAX_NOTIONAL_USD` 是静态常量，而 `main.py` 每日把 `POSITION_SIZE` 动态重置为总资产的 5%——atr_dual 模式下单笔风险不再随账户规模缩放（固定 $40）。开仓前已按实际保证金校验余额，账面不会为负；若将来要恢复"随账户缩放"，把这两个常量改为按资产比例计算即可。
- 回滚方向（atr_dual → fixed）：存量 atr_dual 仓位仍按其记录的硬止损管理，但软止损检查被模式开关禁用——这些仓位在自然平仓前只有较宽的硬止损（最大 6%，约 $80 风险）+ 移动止盈保护，属可接受的过渡性暴露。

## 回测验证结果 (2026-06-11)

数据：29 个主流 symbol（MATICUSDT 已退市无数据），约 1 年 1H/1D K线（2025-06-11 至 2026-06-11 12:00 UTC），仅小时级（无分钟数据——两种模式的硬止损同为收盘判定，对比公平但绝对数字偏乐观）。
除 stop-mode 外全部默认参数（capital=$8,084，position_size=$400，leverage=5x，max_positions=10）。
...
| 总手续费 | $2,752 | $3,109 |
| 胜率 | 35.1% | 38.4% |
| 最大回撤 | $11,472（94.98%） | $8,752（72.90%） |
| Sharpe | −0.44 | +0.06 |
| 平均持仓时长 | 19.3h | 21.9h |
| ≤1h 出场占比 | 12% | 4% |
| 出场原因分布 | fixed_sl 65%，trailing_tp 35% | soft_sl 58%，trailing_tp 39%，hard_sl 3% |

**判定：不通过。**

atr_dual 在所有指标上均优于 fixed（净亏减少 $3,340、最大回撤收窄 22pp、Sharpe 从 −0.44 改善至 +0.06、≤1h 闪亡率从 12% 降至 4%），说明 ATR 自适应止损在减少噪音收割方面有真实作用。但两种模式净 PnL 均为负（整体策略无正 alpha），达不到"净 PnL 显著为正"的上线门槛。atr_dual 的改善是相对的，绝对意义上仍是亏钱策略；手续费总额还因成交笔数增多而上升。切实盘前需先确认策略本身有正 edge，单纯改止损结构无法解决根本问题。
