...
| 类型 | 参数 | 说明 |
|------|------|------|
| 固定止损 | `FIXED_STOP_LOSS_PCT = 2%` | 价格从入场价逆向移动 2% 立即止损 |
| 激活式移动止盈 | `TRAILING_ACTIVATION_PCT = 3%` | 浮盈达到 3% 后激活 |
| 移动止盈回撤 | `TRAILING_DRAWDOWN_PCT = 1%` | 激活后，从最有利价格回撤 1% 出场 |

出场逻辑：固定止损先于移动止盈判断。移动止盈激活后自动追踪最高/最低价，回撤超过 1% 即触发。
...
每 6 小时自动推送心跳汇报，包含：总资产、持仓 PnL、交易统计（胜率、已实现 PnL、手续费）。
